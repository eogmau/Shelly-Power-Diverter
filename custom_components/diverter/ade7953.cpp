...
#include "esphome/core/log.h"
 ...
 void ADE7953::update() {
  ...
  ADE_PUBLISH(active_power_a, (int32_t) val, 300.0f); //f=4862401*current_factor*voltage_factor/(9032007*9032007)
  err = ade_read_32_(0x0313, &val);
  ADE_PUBLISH(active_power_b, (int32_t) val, 154.0f);
  err = ade_read_32_(0x031A, &val);
  ADE_PUBLISH(current_a, (uint32_t) val, 193700.0f); // adjusted, f=9032007/(0,5*ratio_CT/(R*SQRT(2))
  err = ade_read_32_(0x031B, &val);
  ADE_PUBLISH(current_b, (uint32_t) val, 193700.0f); // adjusted, f=9032007/(0,5*ratio_CT/(R*SQRT(2))
  err = ade_read_32_(0x031C, &val);
  ADE_PUBLISH(voltage, (uint32_t) val, 26000.0f);
  ADE_PUBLISH(energy_buffer, energy_buffer_, 1.0f);
  ADE_PUBLISH(energy_diverted, energy_diverted_, 1.0f);


   // Additional values can be enabled:
